...
"#
    ...
    let auth_value: serde_json::Value =
        read_json_file(&tokens_buddy_lib::get_codex_auth_path()).expect("read auth.json");
    assert_eq!(
...
fn switch_packycode_gemini_updates_security_selected_type() {
    ...
    // Gemini security settings are written to ~/.gemini/settings.json, not ~/.tokens-buddy/settings.json
