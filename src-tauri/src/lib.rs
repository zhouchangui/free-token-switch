...
pub fn run() {
    ...
    let builder = builder
        ...
        .setup(|app| {
            ...
            // 初始化日志（单文件输出到 <app_config_dir>/logs/tokens-buddy.log）
            {
                ...
                let log_file_path = log_dir.join("tokens-buddy.log");
                    ...
                    tauri_plugin_log::Builder::default()
                        ...
                        .targets([
                            ...
                            Target::new(TargetKind::Folder {
                                path: log_dir,
                                file_name: Some("tokens-buddy".into()),
                            }),
                        ])
            ...
            let db_path = app_config_dir.join("tokens-buddy.db");
            ...
            //
            // 捕获首次运行快照：所有全新装用户都会看到欢迎弹窗介绍 TokensBuddy 的工作方式。
            ...
            #[cfg(any(target_os = "linux", all(debug_assertions, windows)))]
            {
                #[cfg(target_os = "linux")]
                {
                    ...
                    // tauri-plugin-deep-link writes to: ~/.local/share/com.tokensbuddy.desktop/applications/tokens-buddy-handler.desktop
                    // Only register if .desktop file doesn't exist to avoid overwriting user customizations
                    let should_register = app
                        .path()
                        .data_dir()
                        .map(|d| !d.join("applications/tokens-buddy-handler.desktop").exists())
                        .unwrap_or(true);
...
fn show_migration_error_dialog(app: &tauri::AppHandle, error: &str) -> bool {
    ...
    let message = if is_chinese_locale() {
        ...
        format!(
            ...
            Consider rolling back to an older TokensBuddy version.\n\n\
...
fn show_database_init_error_dialog(
    ...
    let message = if is_chinese_locale() {
        format!(
            ...
            建议：\n\
            1) 先备份整个配置目录（包含 tokens-buddy.db）\n\
        ...
        format!(
            ...
            Suggestions:\n\
            1) Back up the entire config directory (including tokens-buddy.db)\n\
            2) If you see “database version is newer”, please upgrade TokensBuddy\n\
