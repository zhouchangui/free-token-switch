...
pub fn anthropic_to_responses(
    ...
    //   （tokens-buddy 的 transform 当前是"条件写入"，可能产生缺失）
    // - stream: 必须永远 true（codex-rs 硬编码 true，且 tokens-buddy 的
...
mod tests {
    ...
    fn test_codex_oauth_forces_stream_true_even_when_client_sends_false() {
        ...
        // 依据：tokens-buddy SSE 解析层只支持流式响应
        let input = json!({
            "model": "gpt-5-codex",
            "max_tokens": 1024,
            "stream": false,
