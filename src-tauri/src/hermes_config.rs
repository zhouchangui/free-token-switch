...
///
/// Input (from TokensBuddy UI / database):
/// ```json
...
/// `providers:` dict. TokensBuddy treats those as read-only — writes have to
...
/// - `custom_providers:` list entries (writable by TokensBuddy)
...
/// keeping TokensBuddy free to edit it. Models are denormalized from the YAML
...
/// list — i.e. it is a read-only overlay TokensBuddy must not touch.
...
pub fn set_provider(
        ...
        .find(|p| p.get("name").and_then(|n| n.as_str()) == Some(name))
    {
        ...
        // this merge, a TokensBuddy edit to an unrelated field would silently
...
fn scan_hermes_health_internal(content: &str) -> Vec<HermesHealthWarning> {
    ...
    if version >= 12 && providers_dict_populated {
        warnings.push(hermes_warning(
            "schema_migrated_v12",
            "Hermes' newer schema moved some entries into the 'providers:' dict. They are shown read-only in TokensBuddy — edit or remove those entries via Hermes Web UI.".to_string(),
            "providers",
        ));
    }
...
// TokensBuddy fills that gap by reading/writing the whole file as a markdown
...
";
    ...
    fn set_provider_preserves_unknown_fields_on_update() {
        ...
        // Hermes Web UI; a later TokensBuddy edit must not strip them — set_provider
...
";
    ...
    fn set_memory_enabled_preserves_other_fields() {
        ...
        // a TokensBuddy toggle would silently wipe those fields.
        with_test_home(|| {
            let yaml = "\
memory:
