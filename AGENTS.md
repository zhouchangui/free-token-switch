...
5. 为方便后续合并，设计和实现时优先新增独立文件或小模块，尽量少改现有大文件；必须修改时保持补丁最小。
6. 用户可见文案必须走 i18n；改文案时同步更新 `src/i18n/locales/` 三个语言文件。
7. Tauri 命令/API 契约变更时，Rust 与 TypeScript 必须同步。
8. 用最窄但有效的检查验证；无法运行检查时说明原因。
9. 不要声称根项目 `pnpm lint` 通过；当前没有这个脚本。
10. 生成产物和本地状态不得进入提交。
