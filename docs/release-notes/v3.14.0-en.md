...
- **Application-Level Window Controls**: Opt-in setting to render TokensBuddy's own min/max/close buttons, materially improving Linux Wayland experience
- **Hermes in Unified Skills Management**: Skill install, enable, and filter now cover Hermes
- **Hermes / OpenClaw Config Directory Override**: Point TokensBuddy at a custom `~/.hermes/config.yaml` or `openclaw.json` location
...
TokensBuddy now treats Hermes Agent as a first-class managed app alongside Claude / Codex / Gemini / OpenCode / OpenClaw.
...
- **User `providers:` Dict Read-Only Rendering**: User-authored providers in the YAML appear as read-only cards in TokensBuddy; deep configuration delegates to the Hermes Web UI
...
- Opt-in setting to render TokensBuddy's own minimize / toggle-maximize / close buttons instead of system decorations (#1119, thanks @git1677967754)
...
- New settings option to point TokensBuddy at a custom `openclaw.json` location (#1518, thanks @mrFranklin)

### Hermes Config Directory Override

- New settings option to point TokensBuddy at a custom `~/.hermes/config.yaml` location, backed by data-driven dispatch
...
- Deep YAML knobs are no longer duplicated in the TokensBuddy form — they are delegated to the Hermes Web UI via a direct launch action
...
- The tray identifier moved from the hardcoded string `"main"` to a `TRAY_ID` constant (`"tokens-buddy"`) across all call sites (#1978, thanks @lidaxian121)
...
- Round-tripping through TokensBuddy no longer drops OAuth MCP `auth` blocks or pollutes unrelated YAML keys
...
By enabling these features, users **accept all associated risks**. TokensBuddy is not responsible for any account restrictions, warnings, or service suspensions that result from using these features.

---

## Download & Installation

Visit [Releases](https://github.com/zhouchangui/tokens-buddy/releases/latest) to download the appropriate version.
...
| File                                     | Description                                     |
| ---------------------------------------- | ----------------------------------------------- |
| `TokensBuddy-v3.14.0-Windows.msi`          | **Recommended** - MSI installer, supports auto-update |
| `TokensBuddy-v3.14.0-Windows-Portable.zip` | Portable, extract and run, no registry writes   |

### macOS

| File                             | Description                                              |
| -------------------------------- | -------------------------------------------------------- |
| `TokensBuddy-v3.14.0-macOS.dmg`    | **Recommended** - DMG installer, drag into Applications  |
| `TokensBuddy-v3.14.0-macOS.zip`    | Extract and drag into Applications, Universal Binary     |
| `TokensBuddy-v3.14.0-macOS.tar.gz` | For Homebrew installation and auto-update                |
...
```bash
brew tap zhouchangui/tokens-buddy
...
| --------------------------------------- | ----------- | ---------------------------------------------------------------------- |
| Ubuntu / Debian / Linux Mint / Pop!\_OS | `.deb`      | `sudo dpkg -i TokensBuddy-*.deb` or `sudo apt install ./TokensBuddy-*.deb` |
| Fedora / RHEL / CentOS / Rocky Linux    | `.rpm`      | `sudo rpm -i TokensBuddy-*.rpm` or `sudo dnf install ./TokensBuddy-*.rpm`  |
| openSUSE                                | `.rpm`      | `sudo zypper install ./TokensBuddy-*.rpm`                                |
| Arch Linux / Manjaro                    | `.AppImage` | Add execute permission and run, or use AUR                             |
| Other distros / not sure                | `.AppImage` | `chmod +x TokensBuddy-*.AppImage && ./TokensBuddy-*.AppImage`              |
