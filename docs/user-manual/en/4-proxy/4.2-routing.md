...
App routing means letting TokensBuddy route a specific application's API requests through the local routing service.
...
When routing is enabled, TokensBuddy modifies the app's configuration file to point the API endpoint to the local routing service.
...
When disabling routing, TokensBuddy will:
...
Before enabling routing, TokensBuddy backs up the original configuration and restores it when disabled.
