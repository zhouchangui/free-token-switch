...
## TokensBuddy Data Storage

### Storage Directory

Default location: `~/.tokens-buddy/`
...
```
~/.tokens-buddy/
├── tokens-buddy.db      # SQLite database (SSOT)
...
`tokens-buddy.db` is a SQLite database that stores:
...
| Field | Description |
|-------|-------------|
| `models.providers` | Provider configuration (mapped to TokensBuddy's "providers") |
...
TokensBuddy's priority when modifying configurations:

1. **TokensBuddy Database** - Single source of truth (SSOT)
...
- CLI tool configuration files (will be backfilled by TokensBuddy)
- TokensBuddy's `settings.json`

### Not Recommended to Edit Manually

- `tokens-buddy.db` database file
- Backup files
...
TokensBuddy v3.7.0 migrated from JSON files to SQLite:
