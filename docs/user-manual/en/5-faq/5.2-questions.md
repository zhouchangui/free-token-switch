...
TokensBuddy for macOS is code-signed and notarized by Apple. You can download and install it directly without any additional steps. If you encounter issues, try downloading the latest version from the [Releases page](https://github.com/zhouchangui/tokens-buddy/releases).
...
2. Add TokensBuddy to your antivirus software's whitelist
...
chmod +x TokensBuddy-*.AppImage

# If it still fails, try
./TokensBuddy-*.AppImage --no-sandbox
```
...
**Solutions**:
1. Check if the `~/.tokens-buddy/` directory exists
2. Restore from backup: `~/.tokens-buddy/backups/`
...
**Solutions**:
1. Confirm the file was exported by TokensBuddy
...
**Windows**:
- Check taskbar settings to ensure the TokensBuddy icon is not hidden
...
2. Restart the app
3. Delete `~/.tokens-buddy/settings.json` to reset settings
...
3. If using Homebrew: `brew upgrade --cask tokens-buddy`

## Lightweight Mode

### How to Enter Lightweight Mode?

Toggle "Lightweight Mode" from the system tray menu. The main window closes, and TokensBuddy runs as a tray-only app. Toggle again or click "Open main window" to exit.
...
Yes. Starting from v3.13.0, TokensBuddy covers all window re-show paths (normal launch, deep links, singleton activation, tray `show_main`, and Lightweight Mode return). Clicking a `tokensbuddy://` link **rebuilds the main window on demand** and displays the import confirmation dialog. The first open is slightly slower than normal state (window rebuild required), but subsequent switches return to normal speed.
...
1. Visit [GitHub Issues](https://github.com/zhouchangui/tokens-buddy/issues)
...
- macOS/Linux: `~/.tokens-buddy/logs/`
- Windows: `%APPDATA%\tokens-buddy\logs\`
