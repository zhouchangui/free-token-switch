...
## TokensBuddy のデータストレージ

### ストレージディレクトリ

デフォルトの場所：`~/.tokens-buddy/`
...
```
~/.tokens-buddy/
├── tokens-buddy.db      # SQLite データベース（SSOT）
...
`tokens-buddy.db` は SQLite データベースで、以下を保存しています：
...
| フィールド | 説明 |
|------|------|
| `models.providers` | プロバイダー設定（TokensBuddy の「プロバイダー」にマッピング） |
| `env` | 環境変数設定 |
...
TokensBuddy が設定を変更する際の優先順位：

1. **TokensBuddy データベース** - 単一事実源 (SSOT)
...
- CLI ツールの設定ファイル（TokensBuddy がバックフィルする）
- TokensBuddy の `settings.json`

### 手動編集を推奨しないもの

- `tokens-buddy.db` データベースファイル
- バックアップファイル
...
TokensBuddy v3.7.0 で JSON ファイルから SQLite に移行しました：
